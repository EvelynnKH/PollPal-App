...
class SurveyQuestionViewModel: ObservableObject {
    ...
    var progressFraction: Double { Double(currentIndex + 1) / Double(max(questions.count, 1)) }
        ...
        viewContext.performAndWait {
            ...
            for question in questions {
                ...
                if type == "Check Box" {
                    if let selectedSet = multiSelectionAnswers[qID] {
                        for optionID in selectedSet {
                            createDResponse(hRes: hRes, question: question, optionID: optionID, text: nil)
                        }
                    }
                }
                // --- CASE 2: SINGLE SELECT ---
                else if ["Multiple Choice", "Drop Down", "Linear Scale"].contains(type) {
                ...
                // --- CASE 3: TEXT ---
                    ...
                    let text = textAnswers[qID]
                    createDResponse(hRes: hRes, question: question, optionID: nil, text: text)
                }
            }
