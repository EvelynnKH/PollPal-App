...
class SurveyViewModel: ObservableObject {
    ...
    func publishSurvey() -> Bool {
            ...
            if let uuidString = UserDefaults.standard.string(forKey: "logged_in_user_id"),
               let uuid = UUID(uuidString: uuidString) {
                ...
                if let userOwner = try? context.fetch(req).first {
                    ...
                    print("✅ User owner linked successfully in publishSurvey().")
                }
            }
        }
        ...
        guard let owner = survey.owned_by_user else {
            print("❌ Cannot publish: User owner not found after linking attempt.")
        ...
        if owner.user_point < requiredPoints {
            print("⚠️ Publish cancelled: Insufficient user points. Current: \(owner.user_point), Needed: \(requiredPoints)")
    ...
    func deductUserPoints() -> Bool {
        ...
        if currentPoints >= pointsToDeduct {
            ...
            saveContext()
            print("✅ Points deducted successfully. User: \(owner.user_name ?? "Unknown"), New Points: \(owner.user_point)")
            return true
        } else {
            print("❌ Failed to deduct points: Insufficient points. Current: \(currentPoints), Needed: \(pointsToDeduct)")
    ...
    func recordSurveyCostTransaction(for survey: Survey, cost: Int) {
        guard let user = survey.owned_by_user else {
            print("❌ ERROR: User not linked to survey, cannot record transaction.")
            return
        }
        
        // Cek Poin sebelum mengurangi, untuk jaminan ekstra
        if user.user_point < Int32(cost) {
            print("❌ ERROR: User points (\(user.user_point)) are insufficient for cost \(cost).")
        ...
        transaction.transaction_description = "Survey Cost: \(survey.survey_title ?? "Untitled Survey")"
        ...
        do {
            try context.save()
            print("✅ Transaction recorded and User points updated successfully. New point: \(user.user_point)")
            ...
            print("❌ Failed to save transaction and update user points: \(error.localizedDescription)")
            context.rollback()
        }
    }
